...
export class Multiselect extends React.Component {
  ...
  renderSelectedList() {
    ...
    return selectedValues.map((value, index) => (
      <span className={`chip  ${singleSelect && 'singleChip'} ${this.isDisablePreSelectedValues(value) && 'disableSelection'}`} key={index} style={style['chips']}>
        ...
        <FontAwesomeIcon icon={faWindowClose}  className={`icon_cancel closeIcon`} color={"#3976eb"} onClick={() => this.onRemoveSelectedItem(value)}/>
      </span>
    ));
  }
  ...
  renderMultiselectContainer() {
    ...
    return (
      <div className={`multiSelectContainer ${disable ? `disable_ms` : ''}`} id={id || 'multiselectContainerReact'} style={style['multiselectContainer']}>
        <div className={`searchWrapper ${singleSelect ? 'singleSelect' : ''}`} 
